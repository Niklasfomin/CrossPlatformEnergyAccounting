...
df = pd.read_parquet("estimation/data/parallel_bench_replay_2.parquet")
...
# --- Spearman Correlation (vertikal, für Paper) ---
plt.figure(figsize=(8, 3))
spearman_corr_no_target = spearman_corr[target].drop(target)
spearman_corr_no_target.sort_values(ascending=False).plot(
    kind="bar",
    color="#eb9834",
    edgecolor="black"
)
plt.ylabel("Spearman correlation", fontsize=13)
plt.title("Spearman Correlation (Features vs. Interval Energy)", fontsize=14)
plt.xticks(rotation=35, ha='right', fontsize=10)
plt.tight_layout()
plt.grid(axis='y', linestyle=':', alpha=0.4)
plt.show()
