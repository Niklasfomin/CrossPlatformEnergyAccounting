...
class DeltaAggregator:
    def __init__(self, interval=1.0, sample_rate=0.1, db_client=None, meter_client=None, meter_sensor_id="L1"):
    ...
    def get_delta(self):
        ...
        for pid in set(dict1) & set(dict2):
            ...
            delta_io_bytes = self._delta(curr, prev, "disk_io_bytes", clamp_monotonic=True)
            delta_net_send_bytes = self._delta(curr, prev, "net_send_bytes", clamp_monotonic=True)
            delta_syscalls = self._delta(curr, prev, "syscall_count", clamp_monotonic=True)
            delta_ctx_switches = self._delta(curr, prev, "context_switches", clamp_monotonic=True)
            delta_cpu_time_psutil = self._delta(curr, prev, "psutil_cpu_time_ns", clamp_monotonic=True)
            delta_cpu_time_ticks = self._delta(curr, prev, "cpu_time_ticks", clamp_monotonic=True)
            delta_instruction = self._delta(curr, prev, "instructions", clamp_monotonic=True)
            delta_branch_instr = self._delta(curr, prev, "branch_instructions", clamp_monotonic=True)
            delta_cycles = self._delta(curr, prev, "cycles", clamp_monotonic=True)
            delta_cache_misses = self._delta(curr, prev, "cache_misses", clamp_monotonic=True)
            delta_rss_memory = self._delta(curr, prev, "memory_rss_bytes", clamp_monotonic=False)
            # ticks in ns
            ...
            deltas[pid]["syscall_class_deltas"] = {
                cls: int(self._num(curr_classes.get(cls)) - self._num(prev_classes.get(cls)))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run DeltaAggregator and write deltas to InfluxDB")
    parser.add_argument("--influx-url", default=os.getenv("INFLUX_URL", INFLUX_URL), help="InfluxDB URL")
    parser.add_argument("--influx-token", default=os.getenv("INFLUX_TOKEN", INFLUX_TOKEN), help="InfluxDB token")
    parser.add_argument("--influx-org", default=os.getenv("INFLUX_ORG", INFLUX_ORG), help="InfluxDB org")
    parser.add_argument("--influx-bucket", default=os.getenv("INFLUX_BUCKET", INFLUX_BUCKET), help="InfluxDB bucket")
    parser.add_argument("--interval", type=float, default=2.0, help="Aggregation window in seconds (default: 2.0)")
    parser.add_argument("--sample-rate", type=float, default=None, help="Sampling rate in seconds (optional; defaults to interval)")
    parser.add_argument("--meter-host", default=os.getenv("SMARTMETER_HOST"), help="Smart meter host (optional)")
    parser.add_argument("--meter-user", default=os.getenv("SMARTMETER_USER"), help="Smart meter username (optional)")
    parser.add_argument("--meter-password", default=os.getenv("SMARTMETER_PASSWORD"), help="Smart meter password (optional)")
    parser.add_argument("--meter-ssl", action="store_true", help="Enable SSL for smart meter client (optional)")
    parser.add_argument("--meter-sensor-id", default="L1", help="Sensor id to read from smart meter (default: L1)")

    args = parser.parse_args()

    sample_rate = args.sample_rate if args.sample_rate is not None else args.interval

    print(f"Starting DeltaAggregator: interval={args.interval}, sample_rate={sample_rate}")
    print(f"Influx: {args.influx_url} (org={args.influx_org}, bucket={args.influx_bucket})")
