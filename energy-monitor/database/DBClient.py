...
class DBClient:
    ...
    # ):
    def write_deltas(self, timestamp, interval, deltas, node="localhost"):
    ...
    def load_data(self):
        ...
        start, end = self.get_benchmark_window()
        if start is not None and end is not None:
            print(f"Trimming to benchmark window: {start} → {end}")
            df = df[(df["_time"] >= start) & (df["_time"] <= end)]
        else:
