...
class K8sManager:
    def __init__(self, kubeconfig, cgroups: CgroupV2):
    ...
    def get_pod_container_start_events(self, callback=None):
        for event in self.watcher.stream(self.v1.list_pod_for_all_namespaces):
            ...
            print(
                "Event: %s %s"
                % (event_dict["type"], event_dict["object"].metadata.name)
            )
            if event_dict["type"] == "ADDED":
                pod = event_dict["object"]
                # namespace = pod.metadata.namespace
                # pod_name = pod.metadata.name
                for container in pod.spec.containers:
                    # Find the container status that matches this container name
                    ...
                    if container_status and container_status.container_id:
                        # container_id is in the format 'docker://<id>' or 'containerd://<id>'
                        raw_id = container_status.container_id
                        # Extract the actual ID after the scheme
                        container_id = raw_id.split("://", 1)[-1]
                        pod_id = pod.metadata.uid
                        # Log pod id and container id
                        logging.info(
                            "Pod ID: %s, Container ID: %s", pod_id, container_id
                        )
                        # Store in a dict: {pod_id: container_id}
                        ...
                        if callback:
                            callback(pod_id, container_id)
                    else:
                        container_id = None

                    logging.info("Started container ID: %s", container_id)
                # Return the dict after processing all containers in the pod
                return getattr(self, "pod_to_container_id_dict", {})
    ...
    #         self.get_pod_container_deltas_summed(container_name, matching_pids, deltas)
    #         self.get_pod_container_pids_deltas(container_name, matching_pids, deltas)
    ...
    #             # Summed metrics
    #             summed = self.pod_container_to_pids_to_metrics_summed[container_name]
